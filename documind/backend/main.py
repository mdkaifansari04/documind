...
def main():
    ...
    with VectorAIClient(SERVER) as client:
        ...
        print(f"   ✓ Connected to {info.get('title', 'VectorAI')} v{info.get('version', 'unknown')}")
        ...
        for i, query in enumerate(queries, 1):
            ...
            results = client.points.search(COLLECTION, vector=query_embedding, limit=3)
            
            print(f"     Found {len(results)} relevant chunks:")
            context_pieces = []
            for j, result in enumerate(results, 1):
                context_pieces.append(result.payload["text"])
